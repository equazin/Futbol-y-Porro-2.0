-- ============================================================
-- 026_picado_mvp_en_empate.sql
-- En caso de empate, el MVP puede ser cualquier jugador oficial
-- (titular con player_id) que haya participado del partido.
-- Se mantiene: no autovoto, MVP debe ser titular, invitados
-- (player_id NULL) no pueden ser votados, y Gol de la Fecha
-- sigue restringido a jugadores con al menos un gol cargado.
-- ============================================================
...
BEGIN
  ...
  v_gol_goals := COALESCE((v_notas_json->'stats'->p_gol_vote::text->>'goals')::integer, 0);

  IF v_gol_goals <= 0 THEN
    RETURN jsonb_build_object('ok', false, 'message', 'El Gol de la Fecha solo puede votarse entre jugadores que hicieron gol');
  END IF;

  -- MVP: en empate, cualquier titular oficial. Con ganador, solo del equipo ganador.
